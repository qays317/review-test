...
# Mirror Docker Hub image -> ECR (minimal, no KMS config)
# Usage:
#   DOCKERHUB_IMAGE=qaysalnajjad/ecs-wordpress-app:v2.3 AWS_REGION=us-east-1 ECR_REPO_NAME=ecs-wordpress-app ./scripts/push-dockerhub-to-ecr.sh
# Or export env vars then run.
#
# Optional env for Docker Hub auth:
#   DOCKERHUB_USERNAME and DOCKERHUB_TOKEN

DOCKERHUB_IMAGE="${DOCKERHUB_IMAGE:-qaysalnajjad/ecs-wordpress-app:v2.3}"
AWS_REGION="${AWS_REGION:-ca-central-1}"
ECR_REPO_NAME="${ECR_REPO_NAME:-ecs-wordpress-app}"
...
echo "${TARGET}"
# Print the URI to stdout for capture by CI
echo "${TARGET}" > /dev/stdout

# optional: export variable for callers that source the script
